<template>
  <div class="page-container">
    <div class="chat-fields-view">
      <div class="view-header">
        <div class="header-content">
          <h1>Chat Fields</h1>
          <p class="subtitle">Configure which metadata fields appear in the Agent Chatbot customer profile panel</p>
        </div>
    ...
    <div v-else-if="fields.length === 0" class="empty-state">
      <p>No fields configured. Click "Add Field" to create a customer profile field.</p>
    </div>
      ...
      >
        ...
        <div class="field-controls">
          ...
          <button 
            ...
            :disabled="field.displayOrder === sortedFields.length - 1"
          >
            ↓ Move Down
          </button>
        </div>
          ...
          <div class="modal-body">
            ...
            <div class="form-group checkbox-group">
              <div class="checkbox-label">
                ...
                <label for="isEditable">Editable by Agent</label>
              </div>
            </div>
          </div>
...
const fieldForm = ref<ProfileField>({
  fieldLabel: '',
  fieldKey: '',
  metadataVariable: '',
  fieldType: 'TEXT',
...
const loadFields = async () => {
  loading.value = true
  try {
    const response = await getAllCustomerProfileFields()
...
const saveField = async () => {
  ...
  try {
    if (editingField.value?.id) {
      await updateCustomerProfileField(editingField.value.id, fieldForm.value)
    } else {
      await createCustomerProfileField(fieldForm.value)
    }
    await loadFields()
    showFieldDialog.value = false
    resetForm()
...
const editField = (field: ProfileField) => {
...
const deleteField = async (id: number) => {
  ...
  try {
    await deleteCustomerProfileField(id)
    await loadFields()
  } catch (error) {
...
const moveField = async (id: number, direction: 'up' | 'down') => {
  ...
  try {
    await updateCustomerProfileField(field.id!, field)
    await updateCustomerProfileField(targetField.id!, targetField)
    await loadFields()
  } catch (error) {
...
const loadAvailableVariables = async () => {
  ...
  try {
    const response = await getAvailableMetadataVariables()
